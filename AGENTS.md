# Rules
Before committing run the linter `bunx expo lint`, and the formatter `bun format`. When there are errors or warnings fix the them first. You can automatically fix some linter errors/warnings with `bunx expo lint --fix`.
